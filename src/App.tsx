        ...
        <GuidePoint
          ...
          text={
            <>
              ...
              dan <strong>Shopee Affiliate CSV</strong> (data komisi). Di versi Satruk, hasil analisa bisa disimpan ke
              server dan dibuka lagi lewat menu History.
            </>
          }
        />
        <GuidePoint
          ...
          text={
            <>
              Pastikan range tanggal Meta CSV dan Shopee CSV <strong>sama persis</strong>. Kalau Meta hanya 1 hari
              tapi Shopee 7 hari, ROAS yang muncul akan menyesatkan — dashboard akan otomatis kasih peringatan kuning.
    ...
    body: (
      <>
        <GuidePoint
          ...
          text={
            <>
              Pergi ke <strong>Ads Manager → Campaigns</strong>. Pastikan lo berada di level Campaign, bukan Ad Sets
              atau Ads.
            </>
          }
        />
        ...
        <GuidePoint
          ...
          text={
            <>
              ...
              <div className="guideExample">
                <strong>Catatan:</strong> Order dengan status <strong>Dibatalkan</strong> otomatis dikeluarkan. Order{' '}
                <strong>Tertunda</strong> tetap dihitung karena cancel rate hanya sekitar 1–2%.
                <br />
                <br />
                <strong>Soal perbedaan angka komisi:</strong> Wajar kalau total komisi di sini lebih rendah dari angka
                di dashboard Shopee. CSV pakai <em>Komisi Bersih</em> (sudah diverifikasi), dashboard Shopee pakai{' '}
                <em>Komisi Kotor</em> (belum final). Selisihnya mengecil kalau export dilakukan beberapa hari setelah
                periode berakhir.
              </div>
            </>
          }
    ...
    body: (
      <>
        <GuidePoint
          ...
          text={
            <>
              Kartu paling atas nunjukkin spend + PPN, total komisi, net profit, ROAS, ROI, clicks, LP Views, dan
              zero komisi. Ini ringkasan paling cepat buat tahu campaign lagi sehat atau nggak.
            </>
          }
        />
        <GuidePoint
          ...
          text={
            <>
              Grafik Spend vs Komisi dan Net Profit Harian dipakai buat ngeliat pola performa per hari. Kalau ada hari
              spend tinggi tapi komisi turun, biasanya masalahnya ada di kreatif, landing, atau range tanggal.
            </>
          }
        />
        <GuidePoint
          ...
          text={
            <>
              Setiap hasil analisa yang disimpan bakal masuk ke history per email. Jadi kalau halaman ke-reload, lo
              tinggal buka snapshot terakhir tanpa upload ulang file.
            </>
          }
        />
      </>
    ...
    body: (
      <>
        ...
        <GuidePoint
          ...
          text={
            <>
              Match type bisa <strong>exact</strong>, <strong>contains</strong>, atau <strong>unmatched</strong>. Exact
              paling bagus. Kalau contains, cek lagi karena kemungkinan ada nama campaign yang mirip.
            </>
          }
        />
        <GuidePoint
      ...
      <div className="guideModal" role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="guideContent">
          <div className="guideHeaderRow">
            <div className="guideHeaderIndex">1</div>
...
function makeWorkspace(): WorkspaceState {
  return {
    ...
    name: '',
    metaFileName: '',
    shopeeAccounts: [{ id: 1, name: '', fileName: '' }],
    clickFileName: '',
  }
}
      ...
      <main className="uploadPage">
        <div className="uploadShell">
            ...
            <div className="workspaceBody">
                ...
                <label className="outlineButton">
                  ...
                  <input
                    ...
                    onChange={(event) => {
                      ...
                      if (file) setWorkspace((current) => ({ ...current, metaFileName: file.name }))
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setWorkspace((current) => ({
                          ...current,
                          shopeeAccounts: current.shopeeAccounts.map((item) =>
                        ...
                        onChange={(event) => {
                          ...
                          if (file) {
                            setWorkspace((current) => ({
                              ...current,
                              shopeeAccounts: current.shopeeAccounts.map((item) =>
                                item.id === account.id ? { ...item, fileName: file.name } : item,
                              ),
                            }))
                          }
                          event.currentTarget.value = ''
              ...
              <button
                ...
                onClick={() =>
                  setWorkspace((current) => ({
                    ...current,
                    shopeeAccounts: [...current.shopeeAccounts, { id: Date.now(), name: '', fileName: '' }],
                  }))
                }
              >
                <Plus size={13} /> Tambah Akun Shopee
              </button>
              ...
              <div className="uploadCard slim">
                ...
                <label className="outlineButton">
                  ...
                  <input
                    ...
                    onChange={(event) => {
                      const file = event.target.files?.[0]
                      if (file) setWorkspace((current) => ({ ...current, clickFileName: file.name }))
                      event.currentTarget.value = ''
                    }}
                  />
                </label>
          ...
          <button type="button" className="dashedAction workspaceActionStub">
            <Plus size={13} /> Tambah Workspace (Akun Meta baru)
          </button>

          <section className="notesPanel">
            <p>
              <strong>Workspace:</strong> Tiap workspace punya 1 akun Meta + akun Shopee yang dipasangin. Data antar
              workspace dipisah — ROAS dan spend nggak akan nyampur.
            </p>
            <p>
            ...
            <p>
              ...
              dashboard performa Shopee (<em>Komisi Kotor</em>) — ini normal. Shopee butuh waktu untuk memproses dan
              memverifikasi order sebelum masuk ke CSV. Selisih akan mengecil saat order makin banyak yang selesai.
            </p>
          </section>
          ...
          <section className="historyPanel">
            <div className="historyPanelHeader">
              ...
              <button type="button" className="outlineButton refreshButton" onClick={() => void refreshHistory(email)}>
                Refresh
              </button>
            </div>

            <div className="historyItems">
              {runs.length === 0 && <p className="historyEmpty">Belum ada history tersimpan untuk {email}.</p>}
              {runs.map((run) => (
                <div className="historyRow" key={run.id}>
                  <div>
                    ...
                    <div className="historyMetaLine">
                      1 upload tersimpan · Meta {run.analysis.quality.metaRowsUsed} rows · Shopee {run.analysis.quality.shopeeRowsUsed} rows · update{' '}
                      {new Date(run.createdAt).toLocaleString('sv-SE').replace('T', ' ')}
                    </div>
                  </div>
                  <button type="button" className="outlineButton historyOpenButton">
                    Buka
                  </button>
                </div>
              ))}
          ...
          <button type="button" className={ready ? 'analyzeButton ready' : 'analyzeButton'} disabled={!ready}>
            Mulai Analisa →
          </button>
