...
import { listRuns, type SavedRun } from './lib/history'
import './App.css'

type GuideStepId = 'persiapan' | 'meta' | 'shopee' | 'dashboard' | 'atribusi'
type WorkspaceShopeeAccount = { id: number; name: string; fileName: string }
type WorkspaceState = {
  id: number
  name: string
  metaFileName: string
...
const GUIDE_STEPS: Array<{ id: GuideStepId; label: string; title: string; body: React.ReactNode }> = [
  {
    ...
    body: (
      <>
        <GuidePoint number={1} title="Apa yang dibutuhkan" text={<>Dashboard ini menggabungkan dua sumber data: <strong>Meta Ads CSV</strong> (data spend & klik iklan) dan <strong>Shopee Affiliate CSV</strong> (data komisi). Di versi Satruk, hasil analisa bisa disimpan ke server dan dibuka lagi lewat menu History.</>} />
        <GuidePoint number={2} title="Samakan range tanggal" text={<>Pastikan range tanggal Meta CSV dan Shopee CSV <strong>sama persis</strong>. Kalau Meta hanya 1 hari tapi Shopee 7 hari, ROAS yang muncul akan menyesatkan — dashboard akan otomatis kasih peringatan kuning.<div className="guideExample"><strong>Contoh yang benar:</strong> Meta export 24–30 Mar · Shopee export 24–30 Mar<br /><strong>Contoh yang salah:</strong> Meta export 30 Mar · Shopee export 24–30 Mar</div></>} />
        <GuidePoint number={3} title="Format file" text={<>Kedua file harus dalam format <strong>.CSV</strong>. Jangan ubah nama kolom atau format isi file — dashboard membaca nama kolom secara langsung dari header CSV.</>} />
      </>
    ),
  },
  {
    ...
    body: (
      <>
        <GuidePoint number={1} title="Buka Meta Ads Manager" text={<>Pergi ke <strong>Ads Manager → Campaigns</strong>. Pastikan lo berada di level Campaign, bukan Ad Sets atau Ads.</>} />
  ...
  },
  {
    ...
    body: (
      <>
        <GuidePoint number={1} title="Buka panel Shopee Affiliate" text={<>Login ke <strong>affiliate.shopee.co.id</strong> → pilih menu <strong>Komisi</strong> atau <strong>Commission Report</strong>.</>} />
        <GuidePoint number={2} title="Set filter tanggal" text={<>Set range tanggal yang <strong>sama dengan Meta CSV</strong>. Shopee menggunakan <strong>Waktu Pemesanan</strong> (tanggal order) sebagai basis filter.</>} />
        <GuidePoint number={3} title="Export CSV" text={<>Klik tombol <strong>Export</strong> atau <strong>Unduh</strong>. File yang didownload langsung bisa diupload ke dashboard tanpa modifikasi apapun.<div className="guideExample"><strong>Catatan:</strong> Order dengan status <strong>Dibatalkan</strong> otomatis dikeluarkan. Order <strong>Tertunda</strong> tetap dihitung karena cancel rate hanya sekitar 1–2%.<br /><br /><strong>Soal perbedaan angka komisi:</strong> Wajar kalau total komisi di sini lebih rendah dari angka di dashboard Shopee. CSV pakai <em>Komisi Bersih</em> (sudah diverifikasi), dashboard Shopee pakai <em>Komisi Kotor</em> (belum final). Selisihnya mengecil kalau export dilakukan beberapa hari setelah periode berakhir.</div></>} />
      </>
    ),
  },
  {
    ...
    body: (
      <>
        <GuidePoint number={1} title="Kartu KPI" text={<>Kartu paling atas nunjukkin spend + PPN, total komisi, net profit, ROAS, ROI, clicks, LP Views, dan zero komisi. Ini ringkasan paling cepat buat tahu campaign lagi sehat atau nggak.</>} />
        <GuidePoint number={2} title="Chart harian" text={<>Grafik Spend vs Komisi dan Net Profit Harian dipakai buat ngeliat pola performa per hari. Kalau ada hari spend tinggi tapi komisi turun, biasanya masalahnya ada di kreatif, landing, atau range tanggal.</>} />
        <GuidePoint number={3} title="History" text={<>Setiap hasil analisa yang disimpan bakal masuk ke history per email. Jadi kalau halaman ke-reload, lo tinggal buka snapshot terakhir tanpa upload ulang file.</>} />
      </>
    ),
  },
  {
    ...
    body: (
      <>
        <GuidePoint number={1} title="Gunakan nama campaign Meta" text={<>Isi <strong>Tag_link1</strong> di link Shopee dengan nama campaign Meta saat bikin link. Dashboard akan berusaha match Tag_link1 ke nama campaign biar spend bisa dibaca per campaign.</>} />
        <GuidePoint number={2} title="Jenis pencocokan" text={<>Match type bisa <strong>exact</strong>, <strong>contains</strong>, atau <strong>unmatched</strong>. Exact paling bagus. Kalau contains, cek lagi karena kemungkinan ada nama campaign yang mirip.</>} />
        <GuidePoint number={3} title="Kalau ROAS nggak nyambung" text={<>Biasanya karena tag beda penulisan, campaign rename setelah link dibuat, atau range tanggal Meta dan Shopee nggak sinkron. Rapihin Tag_link1 dulu baru baca ROAS per tag.</>} />
      </>
    ),
  },
]
...
function GuidePoint({ number, title, text }: { number: number; title: string; text: React.ReactNode }) {
  return <><div className="guideNumber">{number}</div><div className="guideTitle">{title}</div><div className="guideText">{text}</div></>
}

function Header({ onOpenGuide }: { onOpenGuide: () => void }) {
  return <section className="heroHeader"><div className="heroBrand"><span className="heroLogo"><Menu size={15} strokeWidth={2.4} /></span><div><h1>{APP_NAME}</h1><p>{APP_SUBTITLE}</p></div></div><button type="button" className="guideButton" onClick={onOpenGuide}><CircleHelp size={13} />Panduan</button></section>
}
...
function makeWorkspace(): WorkspaceState {
  return { id: 1, name: '', metaFileName: '', shopeeAccounts: [{ id: 1, name: '', fileName: '' }], clickFileName: '' }
}
