        ...
        <GuidePoint
          ...
          text={
            <>
              ...
              tapi Shopee 7 hari, ROAS yang muncul akan menyesatkan — dashboard akan otomatis kasih peringatan
              kuning.
...
function UploadScreen({
  ...
  const addShopeeAccount = () => {
    updateWorkspace((current) => ({
      ...current,
      shopeeAccounts: [...current.shopeeAccounts, { id: Date.now(), name: '', file: null }],
    }))
  }
  ...
  const updateShopeeAccount = (id: number, patch: Partial<WorkspaceShopeeAccount>) => {
    updateWorkspace((current) => ({
      ...current,
      shopeeAccounts: current.shopeeAccounts.map((account) => (account.id === id ? { ...account, ...patch } : account)),
    }))
  }
        ...
        <section className="workspacePanel">
          <div className="workspaceBar">
            ...
            <span className="workspaceStatus">
              Meta {metaReady ? '✓' : '—'} · Shopee {shopeeCount}/1 <ChevronDown size={12} />
            </span>
          </div>
        ...
        <section className="historyPanel">
          <div className="historyPanelHeader">
            <div>
              ...
              <p>
                Kalau halaman ke-reload, buka snapshot tersimpan tanpa upload ulang file. History tetap per-email dan
                tidak digabung antar periode.
              </p>
            </div>
            <button type="button" className="outlineButton refreshButton" onClick={onRefreshHistory}>
              Refresh
            </button>
          </div>
              ...
              <div className="historyRow" key={run.id}>
                <div>
                  <div className="historyDateLine">
                    <strong>
                      {formatHistoryLabel(run.createdAt)} · {new Date(run.createdAt).toLocaleDateString('sv-SE')}
                    </strong>
                  </div>
...
export default function App() {
  ...
  async function saveCurrentSnapshot() {
    ...
    await saveRun({
      ...
      shopeeFile: primaryWorkspace.shopeeAccounts.filter((item) => item.file).map((item) => item.file?.name || '').join(', ') || 'Shopee CSV',
      ppn: ppn / 100,
      analysis,
    })
    await refreshHistory(email)
